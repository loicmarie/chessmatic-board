...
module.exports = class Piece {
	...
	canMoveTo(position, from, to){
		...
		let moveType = (offsets.attack && pieceTo) ? 'attack' : 'move';

		for(let k=0; k<offsets[moveType].length; k++){
			let hasVisibility = true, 
				i=1;
			while(i<=maxScope && hasVisibility && this.isOnBoardAfterMove(from, offsets[moveType][k][0]*i, offsets[moveType][k][1]*i)){
				if(from.row+(offsets[moveType][k][0]*i) == to.row && from.column+(offsets[moveType][k][1]*i) == to.column){
					isValidMove = true;
				} else if(position.board[from.row+(offsets[moveType][k][0]*i)][from.column+(offsets[moveType][k][1]*i)]){
					hasVisibility = false;
				}
				i++;
			}
